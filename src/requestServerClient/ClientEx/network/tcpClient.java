...
public class tcpClient {
	...
	public tcpClient(/*Window window*/MainActivity main) {
		//this.window = window;
		loginResult = false;
		rthread = null;
		this.main = main;
		...
		while(true) {
			try {
				...
				if(data.matches("^@login.*")) {
					...
					StringTokenizer enterUser = new StringTokenizer(data, ":", false); 
					String name = enterUser.nextToken();
					int y = Integer.parseInt(enterUser.nextToken());
					System.out.println(data + " " + y);
					//main.setMe(new Player(0, imgY[4]));
					System.out.println(name + " " + main.getMe().getName());
					if(name.equals(main.getMe().getName()) && main.getMe() != null)
					{
						System.out.println("나");
						main.setMe(new Player(0, y, name));
					} else {
						System.out.println("상대방");
						main.setOther(new Player(0, y, name));
					}
					
                } else if(data.matches("^@MOVE.*")) {
                	data = data.substring(5, data.length());
					StringTokenizer moveUser = new StringTokenizer(data, ":", false); 
					String name = moveUser.nextToken();
					int x = Integer.parseInt(moveUser.nextToken());
					String stroke = moveUser.nextToken();
					System.out.println("X:" + x);
					if(name.equals(main.getMe().getName()))
					{
						main.getMe().setX(x);
						switch(stroke) {
						case "LEFT":
						...
						main.getOther().setX(x);
						switch(stroke) {
						case "LEFT":
							main.getOther().setStroke(new ImageIcon(MainActivity.class.getResource("res/strokes/freestyle_1.png")).getImage());
							break;
						case "RIGHT":
							main.getOther().setStroke(new ImageIcon(MainActivity.class.getResource("res/strokes/freestyle_3.png")).getImage());
							break;
						case "SPACE":
							main.getOther().setStroke(new ImageIcon(MainActivity.class.getResource("res/strokes/freestyle_2.png")).getImage());
							break;
						}
					}
                }
