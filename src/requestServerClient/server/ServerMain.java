...
public class ServerMain {
	...
	void startServer() { //서버 시작 코드
		...
		try {
			...
			serverSocketChannel.bind(new InetSocketAddress("127.0.0.1", 5001)); // IP(도메인)및 바인딩포트 적용해 서버소켓을 구성
		} catch(Exception e) {
			e.printStackTrace();
			System.out.println("[TCP]이미 사용되고 있는 포트번호 입니다."); //해당 포트를 이미 다른 프로그램에서 사용하고 있을때.
				...
				public void run() {
					...
					while(true) { //무한 반복
						try {
							...
							if(data.matches("^@signup.*")) {
							...
							} else if(data.matches("^@playerEnter.*")) {
		                    	...
		                    	for(Client client : connections)
		                    		for(Player p: players) {
				                    	String msg = "@enterUser/" + p.getName() + ":" + Integer.toString(p.getY());
